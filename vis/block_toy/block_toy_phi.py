...
def main():
    ...
    # get dataset object
    data_loader_phi = get_data(transform, mode='test', num_seq=4, num_sample=5)
    data_loader_F = get_data(transform, mode='test', num_seq=1, num_sample=5)
            ...
            for i in range(len(input_seq)):
                ...
                for j in range(5):
                    image_dir = os.path.join(
                        vpath[i], 'image_%0.5d.jpg' % idx_block[i][4 * j].item())
    ...
    for i in range(adj.shape[0]):
        ...
        f.write("""<td>""")
        f.write("""<center>""")
        query_frames = vpath_phi_list[i].split('/')[-3]
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_frames = vpath_F_list[ngbrs[i][j]].split('/')[-3]

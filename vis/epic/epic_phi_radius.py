...
def get_data(transform, mode='train', num_seq=4, num_sample=5):
    print('[epic_phi_radius.py] Loading data for "%s" ...' % mode)
#     global dataset
    print(args.dataset)
...
def main():
    ...
    # get dataset object
    data_loader_phi = get_data(
        transform, mode='train', num_seq=6, num_sample=1)
    data_loader_F = get_data(transform, mode='train', num_seq=1, num_sample=60)
    ...
    np.save('cache/vpath_phi_list.npy', vpath_phi_list)
    torch.save(idx_block_F_list, os.path.join(
        '.', 'cache/idx_block_F_list.pt'))
    np.save('cache/idx_block_phi_list.npy', vpath_F_list)
    torch.save(idx_block_phi_list, os.path.join(
        '.', 'cache/idx_block_phi_list.pt'))
    ...
    radius_embed = torch.load(os.path.join('cache', 'radius_embed.pt'))

    vpath_phi_list = np.load('cache/vpath_phi_list.npy')
    idx_block_F_list = torch.load('cache/idx_block_F_list.pt')
    vpath_F_list = np.load('cache/idx_block_phi_list.npy')
    idx_block_phi_list = torch.load('cache/idx_block_phi_list.pt')
    ...
    ngbrs_inside = ngbrs[:, 0:args.num_neighbours + 1]
    ngbrs_value_inside = ngbrs_value[:, 0:args.num_neighbours + 1]

    # get indices of neighbors outside the tube and the distance
    outside_idx = []
    for i in range(len(ngbrs)):
        j = 0
        while (ngbrs_value[i][j] < radius_embed[i]):
            j += 1
        outside_idx.append(j)
        ngbrs_outside = []
    ...
    for i in range(len(ngbrs)):
        ...
        for j in range(args.num_neighbours):
            ngbrs_single.append(ngbrs[i][outside_idx[i] + j])
            ngbrs_value_single.append(ngbrs_value[i][outside_idx[i] + j])
        ngbrs_outside.append(ngbrs_single)
        ngbrs_value_outside.append(ngbrs_value_single)

    # plot histogram of L2-distance and predicted radius

    print('Generating L2 distance histograms...')
    for i in tqdm(range(ngbrs.shape[0])):
        fig = plt.hist(ngbrs_value[i], bins=30)
        plt.title('L2-distance Frequency')
        plt.xlabel("value")
        plt.ylabel("Frequency")
        plt.axvline(radius_embed[i], color='r',
                    linestyle='dashed', linewidth=2, label='radius')
        plt.legend()
        plt.savefig("image_grids/hist_l2_%d.png" % i)
        plt.close()
    ...
    for i in tqdm(range(adj.shape[0])):
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_value = str(ngbrs_value_inside[i][j])
            f.write(probe_value)
            f.write("""</center>""")
            f.write(""" </td> """)
        f.write("""</tr>""")
        ...
        for j in range(args.num_neighbours):
            f.write("""<td><img src= """)
            probe_name = "image_F_" + str(ngbrs_inside[i][j]) + ".jpg"
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_frames = vpath_F_list[ngbrs_inside[i][j]].split('/')[-1]
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_frames = str(idx_block_F_list[ngbrs_inside[i][j]][0].item()) + '/' + str(
                idx_block_F_list[ngbrs_inside[i][j]][-1].item())
            #probe_target = str(get_class(target_embed.cpu().numpy()[ngbrs[i][j+1]]))
            f.write(probe_frames)
            f.write("""</center>""")
            f.write("""</td>""")
        ...
        f.write("""<td><img src= """)
        query_name = "hist_l2_" + str(i) + ".png"
        f.write(os.path.join('image_grids', query_name))
        f.write(""" alt="" border=1 height=300 width=450></img></td> """)

        ###############################################################

        # plot neighbours outside tube
        f.write("""<td colspan="6">""")
        f.write("""<table>""")
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_value = str(ngbrs_value_outside[i][j])
            f.write(probe_value)
            f.write("""</center>""")
            f.write(""" </td> """)
        f.write("""</tr>""")
        ...
        for j in range(args.num_neighbours):
            f.write("""<td><img src= """)
            probe_name = "image_F_" + str(ngbrs_outside[i][j]) + ".jpg"
            f.write(os.path.join('image_grids', probe_name))
            f.write(""" alt="" border=3 height=100 width=300></img></td> """)
        f.write("""</tr>""")
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_frames = vpath_F_list[ngbrs_outside[i][j]].split('/')[-1]
            #probe_target = str(get_class(target_embed.cpu().numpy()[ngbrs[i][j+1]]))
            f.write(probe_frames)
            f.write("""</center>""")
            f.write("""</td>""")
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_frames = str(idx_block_F_list[ngbrs_outside[i][j]][0].item()) + '/' + str(
                idx_block_F_list[ngbrs_outside[i][j]][-1].item())
            #probe_target = str(get_class(target_embed.cpu().numpy()[ngbrs[i][j+1]]))
            f.write(probe_frames)
            f.write("""</center>""")
            f.write("""</td>""")

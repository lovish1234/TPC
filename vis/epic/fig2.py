...
parser.add_argument(
    '--test-dir', default='/proj/vondrick/lovish/DPC/dpc/log_tmp/original_k400_128_r18', type=str)
...
def main():
    ...
    with torch.no_grad():
        for idx, (input_seq, target, vpath, frame_first_last) in enumerate(data_loader_pred):
    ...
    with torch.no_grad():
        for idx, (input_seq, target, vpath, frame_first_last) in enumerate(data_loader_GT):
            input_embed = torch.Tensor()
            input_embed = torch.cat((input_embed, input_seq.permute(
                0, 1, 3, 2, 4, 5)[:, :, -1, :, :, :]), 0)
    ...
    for i in range(adj.shape[0]):
        ...
        f.write(os.path.join('image_grids2_pred', query_name))
        f.write(""" alt="" border=3 height=100 width=300></img></td> """)

        for j in range(args.num_neighbours):
            f.write("""<td><img src= """)
            probe_name = "image_" + str(ngbrs[i][j + 1]) + ".jpg"
            f.write(os.path.join('image_grids2_GT', probe_name))
            f.write(""" alt="" border=3 height=100 width=300></img></td> """)
        f.write("""</tr>""")
        ...
        f.write("""<td>""")
        f.write("""<center>""")
        query_target = str(get_class(target_embed_pred.cpu().numpy()[i]))
        ...
        for j in range(args.num_neighbours):
            ...
            f.write("""<center>""")
            probe_target = str(
                get_class(target_embed_GT.cpu().numpy()[ngbrs[i][j + 1]]))
        ...
        f.write("""<td>""")
        f.write("""<center>""")
        query_filename = path_list_pred[i]
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_filename = path_list_GT[ngbrs[i][j + 1]]

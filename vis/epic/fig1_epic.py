...
def get_data(transform, mode='train'):
    print('[fig1_epic.py] Loading data for "%s" ...' % mode)
    global dataset
...
def main():
    ...
    data_loader_pred = get_data(transform, mode='train')
    model = get_model(feature_type='F')

    input_embed_all = torch.Tensor()
    participant_embed_all = torch.Tensor()
    video_embed_all = torch.Tensor()
    ...
    with torch.no_grad():
        for idx, (input_seq, participant, video, action, noun, frame_first_last) in tqdm(enumerate(data_loader_pred), total=len(data_loader_pred)):
            ...
            noun_list.extend(noun_embed)
            verb_list.extend(verb_embed)
            participant_list.extend(participant)
            video_list.extend(video)

            #print(noun_embed, verb_embed)
            ...
            # participant_embed_all = torch.cat(
            #     (participant_embed_all, participant_embed), 0)
            # video_embed_all = torch.cat(
            #     (video_embed_all, video_embed), 0)
    ...
    for i in range(adj.shape[0]):
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_noun = noun_list[ngbrs[i][j + 1]]
            f.write(probe_noun)
            f.write("""</center>""")
            f.write("""</td>""")
        f.write("""</tr>""")
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_verb = verb_list[ngbrs[i][j + 1]]
            f.write(probe_verb)
            f.write("""</center>""")
            f.write("""</td>""")
        f.write("""</tr>""")
        ...
        f.write("""<td>""")
        f.write("""<center>""")
        query_participant = participant_list[i]
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_participant = participant_list[ngbrs[i][j + 1]]
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_video = video_list[ngbrs[i][j + 1]]
            #probe_target = str(get_class(target_embed.cpu().numpy()[ngbrs[i][j+1]]))
            f.write(probe_video)
            f.write("""</center>""")
            f.write("""</td>""")

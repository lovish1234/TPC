...
def get_data(transform, mode='train'):
    print('[epic_F.py] Loading data for "%s" ...' % mode)
#     global dataset
    print(args.dataset)
...
#     video_list = []
    ...
    header = """ <!DOCTYPE HTML>
                 ...
                 <body>
                 <h2>TPC Nearest Neighbours (Function F)</h2>
                 <table>
                 <tr>
                     <th>S.No.</th>
                     <th>Query</th>
...
#             print(probe_name)
        ...
        f.write("""<td>""")
        f.write("""<center>""")
        query_frames = vpath_list[i].split('/')[-1]
        ...
        for j in range(args.num_neighbours):
            f.write("""<td>""")
            f.write("""<center>""")
            probe_frames = vpath_list[ngbrs[i][j + 1]].split('/')[-1]

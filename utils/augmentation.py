...
class Scale:
    ...
    def __call__(self, imgmap):
        ...
        # scale the smaller side to desired value,
        # larger value scaled accordingly, keeping the aspect ratio fixed
        img1 = imgmap[0]
...
class CenterCrop:
    ...
    def __call__(self, imgmap):
        # get a center crop of a particular size
        img1 = imgmap[0]
        w, h = img1.size
...
class RandomCropWithProb:
    ...
    def __call__(self, imgmap):
        ...
        if self.size is not None:
            ...
            # if the image is of crop-size, return
            if w == tw and h == th:
                return imgmap
            if self.consistent:
                # what is consistancy
                # every image in the batch should be cropped at the same point
...
class RandomCrop:
    ...
    def __call__(self, imgmap, flowmap=None):
        ...
        # flowmap - uses optical flow to crop the images
        # choose the crop with the largest optical flow
        img1 = imgmap[0]
        w, h = img1.size
        if self.size is not None:
            th, tw = self.size
    ...
    def __call__(self, imgmap):
        ...
        if random.random() < self.threshold:  # do RandomSizedCrop
            for attempt in range(10):
                ...
                # select random sized area and random aspect ratio
                target_area = random.uniform(0.5, 1) * area
                aspect_ratio = random.uniform(3. / 4, 4. / 3)

                # set w and h accordingly
            ...
            # Fallback [ if the above operations not possible, do this]
...
class RandomSizedWideCrop:
    '''
    Random crops that are 1.5x wider on average. Consistent only.
    Fallback: select either full video or random square crop.
    '''

    def __init__(self, size, interpolation=Image.BILINEAR, p=1.0):
        self.size = size
        self.interpolation = interpolation
        self.threshold = p
        self.fallback_crop = RandomCrop(size, consistent=True)
        self.fallback_scale = Scale((size, size), interpolation=interpolation)
    ...
    def __call__(self, imgmap):
        ...
        if random.random() < self.threshold:
            for attempt in range(10):
                ...
                if random.random() < 0.5:
                    w, h = h, w
                # NOTE: modified here, width is often larger => factor ~1.5
                w = int(w * 1.2)
                h = int(h * 0.8)
                if w <= img1.size[0] and h <= img1.size[1]:
                    x1 = random.randint(0, img1.size[0] - w)
        ...
        if random.random() < 0.5:
            # Random square crop
            return self.fallback_crop(imgmap)
        else:
            # Full video
            return self.fallback_scale(imgmap)
...
class RandomGray:
    ...
    def grayscale(self, img):
        '''
        Pick up a random channel and replicate it
        '''

...
parser.add_argument('--prefix', default='new', type=str,
...
# loss function
parser.add_argument('--loss-function', default='CE',
                    type=str, help='can be CE(Cross-entropy) or MSE(Mean-Squared Error)')
# distance (score function)
parser.add_argument('--distance', default='L2',
                    type=str, help='can be cosine or L2')
# distance function type ( certain or uncertain )
parser.add_argument('--distance-type', default='uncertain',
                    type=str, help='can be certain or uncertain')
# treat positive and negative instances same or different
parser.add_argument('--positive-vs-negative', default='same',
                    type=str, help='can be same or different')
# predict log(r) instead of (r) [only valid if the distance is certain]
parser.add_argument('--radius-type', default='linear',
                    type=str, help='can be linear or log')
# which radius to use [ predicted or ground truth ]
parser.add_argument('--radius-which', default='pred',
                    type=str, help='can be gt or pred')
parser.add_argument('--num_workers', default=32, type=int)


# get embeddings
parser.add_argument('--get-embeddings', default='Yes',
                    type=str, help='get tensorboard embeddings')
...
def main():
    ...
    if args.model == 'dpc-rnn':
        model = DPC_RNN(sample_size=args.img_dim,
                        ...
                        positive_vs_negative=args.positive_vs_negative,
                        distance_type=args.distance_type,
                        radius_type=args.radius_type,
                        radius_which=args.radius_which)
    else:
    ...
    if args.dataset == 'ucf11':  # designed for ucf101, short size=256, rand crop to 224x224 then scale to 128x128
        transform = transforms.Compose([
            RandomHorizontalFlip(consistent=True),
            RandomCrop(size=224, consistent=True),
            Scale(size=(args.img_dim, args.img_dim)),
    ...
    elif args.dataset in ['epic', 'epic_gulp']:
        transform = transforms.Compose([
            RandomHorizontalFlip(consistent=True),
            RandomCrop(size=224, consistent=True),
            Scale(size=(args.img_dim, args.img_dim)),
    ...
    elif args.dataset in ['block_toy_combined_train',
                          'block_toy_combined_test']:
        # may have to introduce more transformations with imagenet background
        transform = transforms.Compose([
            # centercrop
            # CenterCrop(size=224),
...
def get_data(transform, mode='train'):
    print('[main.py] Loading data for "%s" ...' % mode)
    ...
    if mode == 'train':
        ...
        data_loader = data.DataLoader(dataset,
                                      ...
                                      pin_memory=True,
                                      drop_last=True)
    print('[main.py] "%s" dataset size: %d' % (mode, len(dataset)))
    return data_loader
...
train-{args.train_what}{2}_loss-function-{args.loss_function}_distance-{args.distance}_distance-type_{args.distance_type}_\
positive_vs_negative_{args.positive_vs_negative}_radius-type-{args.radius_type}_radius-which-{args.radius_which}'.format(
            'r%s' % args.net[6::],

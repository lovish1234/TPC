...
class ResNet2d3d_full(nn.Module):
    def __init__(self, block, layers, track_running_stats=True, **kwargs):
        super(ResNet2d3d_full, self).__init__()

        if 'distance_type' in kwargs.keys():
            self.distance_type = kwargs['distance_type']
        else:
            self.distance_type = 'certain'

        if 'radius_type' in kwargs.keys():
            self.radius_type = kwargs['radius_type']
        else:
            self.radius_type = 'linear'
        ...
        if self.distance_type == 'uncertain':
            # add one channel for predicting radius of circle
            print('[resnet_2d3d.py] Using distance metric : uncertain')
            self.layer4 = self._make_layer(
                block[3], 257, layers[3], stride=2, is_final=True)
        else:
            print('[resnet_2d3d.py] Using distance metric : certain')
            self.layer4 = self._make_layer(
        ...
        if self.radius_type == 'log' and self.distance_type == 'uncertain':
            print('[resnet_2d3d.py] Setting log as radius_type')
            self.activation = exp_activation()
        elif self.radius_type == 'linear':
            print('[resnet_2d3d.py] Setting linear as radius_type')
    ...
    def forward(self, x):
        ...
        # make the radius positive using exponential function
...
class ResNet2d3d_mini(nn.Module):
    def __init__(self, block, layers, track_running_stats=True, **kwargs):
        ...
        if self.distance_type == 'uncertain':
            ...
            print('[resnet_2d3d.py] Using distance metric : uncertain')
            self.layer4 = self._make_layer(
                block[1], 17, layers[1], stride=2, is_final=True)
        else:
            print('[resnet_2d3d.py] Using distance metric : certain')
            self.layer4 = self._make_layer(
...
if __name__ == '__main__':
    mymodel = resnet18_2d3d_full(distance_type='uncertain', radius_type='log')

...
def save_video(metadata_chunk):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df.index)):
        ...
        output_base_directory = '/proj/vondrick/datasets/epic-kitchens/data/videos/rgb/'
        ...
        output_filename = os.path.join(
            output_directory, row['video_id'] + '_' + row['verb'] + '_' + row['noun'] + '_' + str(start_frame) + '_' + str(stop_frame) + '.mp4')

        # try writing the file as mp4
        writer = FFmpegWriter(output_filename, inputdict={'-r': str(fps)})
        for i in range(row['start_frame'], row['stop_frame'] + 1):
